...
from .scripts.pgap_samplesheet import pgap_dir_to_samplesheet
import yaml
...
@main.command(name="pgap-samples",
             help="Generate a PGAP samplesheet from a directory of genome FASTAs.")
@click.option("-i", "--input-dir", required=True,
              help="Directory of genome FASTAs (scanned recursively).")
@click.option("-o", "--sample-file", default="configs/pgap_samples.csv", show_default=True,
              help="Output samplesheet CSV (name,fasta,taxon).")
@click.option("--taxon", default="",
              help="Default taxon (-s) applied to every row; edit the CSV "
                   "afterward for genomes that need a different value.")
@click.option("--pattern", multiple=True,
              help="Glob pattern to match (repeatable); default "
                   "*.fa/*.fasta/*.fna/*.fas.")
def pgap_samples(input_dir, sample_file, taxon, pattern):
    click.echo("Generating PGAP samplesheet from genome directory...")
    assemblies = pgap_dir_to_samplesheet(
        input_dir, sample_file, taxon=taxon, pattern=list(pattern) or None)
    click.echo(f"Wrote {len(assemblies)} genome(s) to {sample_file}")
...
# PGAP
@main.command()
@click.option('--config', '-c', default='configs/pgap_config.yaml', help='Configuration File')
@click.option('--local',  is_flag=True, help="Run on local machine")
@click.option('--no-conda',  is_flag=True, help="Do not use conda, under construction")
@click.option('--dry',  is_flag=True, help="Show commands without running them")
@click.option('--cores', '-j', type=int, default=None,
              help="Cores for a local run / concurrent jobs on the cluster.")
@click.option('--nodelist', '-w', default=None,
              help="Pin every PGAP job to this SLURM node "
                   "(must belong to --partition).")
@PARTITION_OPTION
def pgap(config, local, dry, no_conda, cores, nodelist, partition):
    """Annotate genomes with NCBI's PGAP.

    Genomes come from the samplesheet generated by `nccrPipe pgap-samples`
    (config key `pgap_samples`), not from samples.csv.
    """
    click.echo("Running PGAP Annotation Pipeline")
    click.echo(f"Config file: {config}")
    click.echo("Running {}".format(
        'locally' if local else ('dry' if dry else 'on cluster')))
    smk_file = "Snakefile"
    cmd = snakemake_cmd(config, 'run_pgap', smk_file, dry, local, no_conda,
                        partition, cores=cores,
                        nodelist=nodelist, single_node=True)
    click.echo(" ".join(cmd))
...
def snakemake_cmd(config, analysis, smk_file, dry, local, no_conda=False,
                  ...
                  latency_wait=60, nodelist=None, single_node=False):
    ...
    if dry:
        ...
        cmd.append(slurm_submit_cmd(partition, nodelist, single_node))
        cmd += shlex.split(
...
def slurm_submit_cmd(partition, nodelist=None, single_node=False):
    ...
    single_node: request one task with N cores on ONE node -- the correct shape
    for a single multithreaded process (PGAP). The default keeps the historical
    `-n {threads}` (N tasks), which SLURM may spread across nodes.
    nodelist: pin every job to this node (must belong to `partition`).
    """
    task_spec = ('-N 1 --ntasks=1 --cpus-per-task={threads}'
                 if single_node else '-n {threads}')
    node_spec = f' --nodelist={nodelist}' if nodelist else ''
    return (
        'DIR=$(dirname {params.qoutfile}); mkdir -p "$DIR"; '
        'sbatch -t {params.time} --mem-per-cpu={params.mem} ' + task_spec + ' '
        '-o {params.qoutfile} -e {params.qerrfile} '
        f'--partition {partition}{node_spec}'
    )
